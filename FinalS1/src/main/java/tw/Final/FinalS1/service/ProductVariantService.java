...
public class ProductVariantService {
	...
	// 根據商品ID、顏色ID、尺寸ID查詢對應的變體
	public ProductVariant getVariantByAttributes(Long productId, Long colorId, Long sizeId) {
		ProductVariant variant = productVariantRepository.findByProductIdAndColorIdAndSizeId(productId, colorId,
				sizeId);
		if (variant == null) {
			throw new RuntimeException("找不到對應的變體");
		}
		return variant;
	}
}
